...
#			print("Setting the last_tranasction_id to be " + str(TransactionManager.last_transaction_id))
	...
	def restartRecovery():
		...
		for rec in allrecords:	
			if rec.info[1] == LogRecord.CHECKPOINT:
				undo_list = rec.info[2]
				check_indx = i			
			i +=1
		
		spliced_arr = allrecords[check_indx+1:len(allrecords)]
		ind =0
	
		for rec in spliced_arr:
			if rec.info[1] == LogRecord.START:
				undo_list.append(rec.info[0])
							
			elif rec.info[1] == LogRecord.ABORT or rec.info[1] == LogRecord.COMMIT:			
				undo_list.remove(rec.info[0])
			
			else:
				#print("REDOING UPDATE {}/{}/{}".format(lr.info[2], lr.info[4], lr.info[6]))
				rel = Relation.getRelationByName(rec.info[2])
				Relation.getRelationByName(rec.info[2]).getTuple(rec.info[3]).setAttribute(rec.info[4],rec.info[-1])
			ind +=1
		...
		for rec in reversed(allrecords):
			if rec.info[1] == LogRecord.START and rec.info[0] in undo_list:
				lr = LogRecord([rec.info[0],LogRecord.ABORT])
			...
			if rec.info[1] == LogRecord.UPDATE and rec.info[0] in undo_list:
				rel = Relation.getRelationByName(rec.info[2])
				
				Relation.getRelationByName(rec.info[2]).getTuple(rec.info[3]).setAttribute(rec.info[4],rec.info[-2])
				print('me')
				lr = LogRecord([rec.info[0],LogRecord.CLR,rec.info[2],rec.info[3],rec.info[4],rec.info[-2]])
				LogManager.writeLogRecord(lr)
...
#		print("REDOING {}".format(transactionid))
