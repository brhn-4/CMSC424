...
def createDatabase1(name):
	...
	instructor.insertTuple(Tuple(instr_schema, ('15151', 'Mozart', 'Magic', '40000', 'Iribe')));
	...
	department.insertTuple(Tuple(dept_schema, ('Ethics', 'Watson', '90000')));
	...
	rs_schema = ["r", "s"]
	rs = db.newRelation("rs", rs_schema)
	rs.insertTuple(Tuple(rs_schema, ('1', '2')));
	rs.insertTuple(Tuple(rs_schema, ('1', '2')));
	rs.insertTuple(Tuple(rs_schema, ('2', '2')));
	rs.insertTuple(Tuple(rs_schema, ('3', '3')));
	rs.insertTuple(Tuple(rs_schema, ('4', '3')));

	st_schema = ["s", "t"]
	st = db.newRelation("st", st_schema)
	st.insertTuple(Tuple(st_schema, ('2', '1')));
	st.insertTuple(Tuple(st_schema, ('2', '2')));
	st.insertTuple(Tuple(st_schema, ('5', '1')));
	st.insertTuple(Tuple(st_schema, ('6', '2')));
