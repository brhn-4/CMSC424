...
def query6a():
	scan1 = SequentialScan(db1.getRelation("instructor"))
	scan2 = SequentialScan(db1.getRelation("department"))
	j = SortMergeJoin(scan1, scan2, "dept_name", "dept_name", SortMergeJoin.INNER_JOIN)
	print("==================== Executing An Inner Join ================")
	j.init()
	for t in j.get_next():
		print("---> " + str(t))
...
def query6b():
	scan1 = SequentialScan(db1.getRelation("instructor"))
	scan2 = SequentialScan(db1.getRelation("department"))
	j = SortMergeJoin(scan1, scan2, "dept_name", "dept_name", SortMergeJoin.FULL_OUTER_JOIN)
	print("==================== Executing A Full Outer Join ================")
	j.init()
	for t in j.get_next():
		print("---> " + str(t))
...
def query6c():
	scan1 = SequentialScan(db1.getRelation("instructor"))
	scan2 = SequentialScan(db1.getRelation("department"))
	j = SortMergeJoin(scan1, scan2, "dept_name", "dept_name", SortMergeJoin.SEMI_JOIN)
	print("==================== Executing A Semi-Join ================")
	j.init()
	for t in j.get_next():
		print("---> " + str(t))

def query6d():
	scan1 = SequentialScan(db1.getRelation("rs"))
	scan2 = SequentialScan(db1.getRelation("st"))
	j = SortMergeJoin(scan1, scan2, "s", "s", SortMergeJoin.SEMI_JOIN)
	print("==================== Executing A Semi-Join ================")
	j.init()
	for t in j.get_next():
		print("---> " + str(t))
...
def query6e():
	scan1 = SequentialScan(db1.getRelation("instructor"))
	scan2 = SequentialScan(db1.getRelation("department"))
	j = SortMergeJoin(scan1, scan2, "dept_name", "dept_name", SortMergeJoin.ANTI_JOIN)
	print("==================== Executing An Anti-Join ================")
	j.init()
	for t in j.get_next():
		print("---> " + str(t))
...
def query6f():
	scan1 = SequentialScan(db1.getRelation("rs"))
	scan2 = SequentialScan(db1.getRelation("st"))
	j = SortMergeJoin(scan1, scan2, "s", "s", SortMergeJoin.ANTI_JOIN)
	print("==================== Executing An Anti-Join ================")
	j.init()
	for t in j.get_next():
		print("---> " + str(t))
...
def query6():
	scan1 = SequentialScan(db1.getRelation("instructor"))
	scan2 = SequentialScan(db1.getRelation("department"))
	j = NaturalJoin(scan1, scan2)
	print("==================== Executing A Natural Join ================")
	j.init()
	for t in j.get_next():
		print("---> " + str(t))
